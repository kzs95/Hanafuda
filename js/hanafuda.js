        ...
        const mutateObserveCallback = function (mutateRecordArr, observer) {
            mutateRecordArr.forEach((mutation) => {
                if (mutation.target.classList.contains("picked")) {
                    console.warn("Mutation Observer Callback")
                    document.body.dispatchEvent(pickCard);
                }
            });
        }
...
class Hanafuda extends CardGame {
    ...
    constructor(gameOptions, playerList, playArea, totalRounds = 12) {
        ...
        playerList.forEach((player)=>player.setAsParticipantOfGame(this));
    ...
    koiKoi(playerWithYaku) {
        ...
        if (playerWithYaku instanceof CPU) { //[CPU Trial]
            const cpuContinue = playerWithYaku.cpuKoiKoiDecision(); //return a random Boolean
...
class Player extends CardHoldingEntity {
    ...
    constructor(areaInDOM, name = "Player", initialPoints = 0) {
        ...
        this.#participantOfGame = undefined;
    }
    ...
    participantOf(){
        return this.#participantOfGame;
    }

    setAsParticipantOfGame(gameInstance){
        this.#participantOfGame = gameInstance;
    }
...
class CPU extends Player { //[CPU Trial]
    ...
    cpuKoiKoiDecision() {
        // previously pass a Hanafuda instance via gameInstance parameter, now have access to it via participantOf() ins method of Player [14 Sep]
        console.warn("CPU making KoiKoi Decision!");
